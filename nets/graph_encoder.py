...
class GCAPCN(nn.Module):
    ...
    def __init__(self,
        ...
        self.n_dim = n_dim  # 节点嵌入的维度
        self.n_p = n_p  # 深度
        self.n_K = n_K
        self.node_dim = node_dim  # 节点的特征维度
        self.init_embed = nn.Linear(node_dim, n_dim * n_p)
        self.init_embed_depot = nn.Linear(2, n_dim)  # 为depot节点初始化，从2维到128维
        ...
        self.normalization_1 = Normalization(n_dim * n_p)  # 归一化层

        self.W_F = nn.Linear(n_dim * n_p, n_dim)  # 最后一层全连接层

        self.activ = nn.LeakyReLU()  # 使用 LeakyReLU 激活函数

    def forward(self, data, mask=None):
        # 将位置、截止时间、工作量拼接在一起，形成一个包含多个特征的矩阵
        # data['loc']维度[2, 100, 2], data['deadline']维度[2, 100]，data['workload']维度[2, 100]
        ...
        # 第一层图卷积计算
        # Layer 1，p = 3
        F0 = self.init_embed(X)  # 【2，100，384】
        ...
        F1 = self.normalization_1(F1)  # 【2，100，384】
        ...
        return (
            h,  # (batch_size, graph_size, embed_dim)
            h.mean(dim=1),  # 沿着指定维度1求平均的操作，最终变为【2，128】
